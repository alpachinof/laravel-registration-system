...
<body dir="rtl">
  @include('alerts')
    <div class="mt-16">
            <form action="/student/<?php echo $student[0]->id; ?>" method="POST">
              @csrf
                  ...
                  <div class="grid grid-cols-6 gap-6">
                    ...
                    <div class="col-span-6 sm:col-span-4">
                        ...
                        <select id="degree" name="degree" class="h-10 mt-1 block w-full rounded-md border-2 border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                          <option {{($student[0]->degree)=="diploma"? 'selected':''}} value="diploma">دیپلم</option>
                          <option {{($student[0]->degree)=="associate"? 'selected':''}} value="associate">کاردانی</option>
                          <option {{($student[0]->degree)=="bachlor"? 'selected':''}} value="bachlor">کارشناسی</option>
                          <option {{($student[0]->degree)=="graduate"? 'selected':''}} value="graduate">ارشد</option>
                          <option {{($student[0]->degree)=="professional"? 'selected':''}} value="professional">دکترا</option>
                        </select>
                      </div>
